...
4. **已於 2026-09-10 確認並實作**：主揪只能修改尚未截止的 `open` 活動名稱與既有候選日期／時間。候選地點、活動類型、預算、地區及截止時間固定，不提供延長期限。`PATCH /api/activities/{id}` 依候選 ID 更新，不新增／刪除或重新排序選項。未來建立投票模型時須先處理日期變更與既有回覆的關係，不能將舊票直接套用到新日期。
